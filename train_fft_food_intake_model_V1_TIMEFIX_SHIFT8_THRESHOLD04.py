...
What this script does:
  ...
  5. Labels each window using fallback annotation rule:
         food = (Chew Count > 0) OR (Chew Bout > 0) OR (Bite > 0)
...
def load_annotation_csv(csv_path: Path, base_date: datetime.date) -> Dict[str, pd.Series]:
    ...
    # Official first-draft fallback label rule requested by user.
    food = ((chew_count > 0) | (chew_bout > 0) | (bite > 0)).astype(float)
...
def build_dataset(conn, sessions: Sequence[AnnotationSession]) -> Tuple[np.ndarray, np.ndarray, List[str], List[WindowRecord]]:
    ...
    for session in sessions:
        ...
        annotation_data = load_annotation_csv(session.csv_path, base_date=sensor_start.date())
    ...
    return X, y, participant_list, records

def save_dataset_cache(X: np.ndarray, y: np.ndarray, participants: List[str], records: List[WindowRecord], path: Path) -> None:
...
def load_dataset_cache(path: Path) -> Tuple[np.ndarray, np.ndarray, List[str], List[dict]]:
    data = np.load(path, allow_pickle=True)
    X = data["X"].astype(np.float32)
    y = data["y"].astype(np.float32)
    participants = [str(p) for p in data["participants"]]
    records = [json.loads(str(r)) for r in data["records"]]
    return X, y, participants, records
...
def main():
    ...
    if CACHE_PATH.exists() and not args.rebuild_cache:
        ...
        X, y, participant_per_window, records = load_dataset_cache(CACHE_PATH)
    else:
        ...
        try:
            X, y, participant_per_window, records = build_dataset(conn, sessions)
        finally:
            conn.close()
        save_dataset_cache(X, y, participant_per_window, records, CACHE_PATH)
